...
def validate_document_server(text):
    """Server kodunda doküman validasyonu - AT Uygunluk Beyanı için"""
    
    # AT Uygunluk Beyanı raporlarında MUTLAKA olması gereken kritik kelimeler
    critical_terms = [
    ...
    for i, category in enumerate(critical_terms):
        ...
        for term in category:
            if re.search(rf"\b{term}\b", text, re.IGNORECASE):
                ...
                logger.info(f"AT Kategori {i+1} bulundu: '{term}'")
                break
...
def check_strong_keywords_first_pages(filepath):
    """İlk 1-2 sayfada özgü kelimeleri OCR ile ara - AT Uygunluk Beyanı için"""
    strong_keywords = [
        "uygunluk",
        "beyan",
        "muayene",
    ...
    try:
        if 'file' not in request.files:
            return jsonify({
                ...
                'message': 'Lütfen analiz edilmek üzere bir AT uygunluk beyanı sağlayın'
            }), 400
        ...
        try:
            ...
            file.save(filepath)
            logger.info(f"AT Uygunluk Beyanı kontrol ediliyor: {filename}")

            # Create analyzer instance
            analyzer = ATTypeInspectionAnalyzer()
            
            # ÜÇ AŞAMALI AT KONTROLÜ
            logger.info(f"Üç aşamalı AT kontrolü başlatılıyor: {filename}")
            file_ext = os.path.splitext(filepath)[1].lower()

            if file_ext == '.pdf':
                # PDF için üç aşamalı kontrol (OCR dahil)
                logger.info("Aşama 1: İlk sayfa AT özgü kelime kontrolü...")
                if check_strong_keywords_first_pages(filepath):
                    logger.info("✅ Aşama 1 geçti - AT özgü kelimeler bulundu")
                else:
                    logger.info("Aşama 2: İlk sayfa excluded kelime kontrolü...")
                    if check_excluded_keywords_first_pages(filepath):
                        logger.info("❌ Aşama 2'de excluded kelimeler bulundu - AT değil")
                        try:
                            os.remove(filepath)
                        except:
                            pass
            ...
            # Buraya kadar geldiyse AT uygunluk beyanı, şimdi analizi yap
            ...
            # Extract key results for API response - AT FORMATINDA
            ...
            response_data = {
                ...
                'analysis_id': f"at_declaration_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                'category_scores': {},
                'date_validity': {
                    'is_valid': True,  # AT uygunluk beyanları için tarih geçerliliği kontrolü uygulanmaz
                    'message': 'AT uygunluk beyanları için tarih geçerliliği kontrolü uygulanmaz',
                    'days_old': 0,
                    'formatted_date': 'N/A'
                },
                'extracted_values': {
            ...
            # Add category scores - AT FORMATINDA
            ...
            return jsonify({
                ...
                'service_description': 'AT Uygunluk Beyanı Analizi',
            ...
            return jsonify({
                'error': 'Analysis failed',
                'message': f'AT uygunluk beyanı analizi sırasında hata oluştu: {str(analysis_error)}',
                'details': {
...
def get_conclusion_message_at(status, percentage):
    """Sonuç mesajını döndür - AT için"""
    if status == "PASS":
        return f"AT uygunluk beyanı 2006/42/EC Makine Direktifi gereksinimlerine uygundur (%{percentage:.0f})"
    elif status == "CONDITIONAL":
        return f"AT uygunluk beyanı kabul edilebilir ancak bazı eksiklikler var (%{percentage:.0f})"
    else:
        return f"AT uygunluk beyanı direktif gereksinimlerine uygun değil, kapsamlı revizyon gerekli (%{percentage:.0f})"

def get_main_issues_at(report):
    """Ana sorunları listele - AT için"""
    issues = []
    
    # Kritik eksiklikler varsa onları ekle
    if report['scoring']['critical_missing']:
        for critical_item in report['scoring']['critical_missing']:
            issues.append(f"Kritik eksik: {critical_item}")
    ...
    if not issues:
        if report['summary']['total_score'] < 50:
            issues = [
                "Üretici adı ve adresi bilgileri eksik",
                "Makine tanımı ve model bilgileri yetersiz",
                "2006/42/EC direktif referansı eksik",
                "Yetkili kişi imzası ve unvanı eksik",
                "Uyumlaştırılmış standart referansları eksik"
            ]
...
def api_info():
    ...
    return jsonify({
        ...
        'endpoints': {
            ...
            'POST /api/at-validate': {
                'description': 'Hızlı geçerlilik kontrolü',
                'response': 'Basit validasyon sonucu'
            }
        },
        'analysis_categories': {
            'Kritik Bilgiler': {
        ...
        'scoring': {
            'VALID': 'Kritik eksiklik yok ve ≥70% - 2006/42/EC uygun',
            'CONDITIONAL': 'Kritik eksiklik yok ve 50-69% - Kabul edilebilir',
            'INSUFFICIENT': 'Kritik eksiklik yok ama <50% - Yetersiz',
            'INVALID': 'Kritik eksiklikler mevcut - Geçersiz'
        }
    })
...
if __name__ == '__main__':
    ...
    assigned_port = int(os.environ.get('ASSIGNED_PORT', 5009))
