...
from Topraklama import EN60204TopraklamaRaporuAnaliz
import logging
...
def validate_document_server(text):
    ...
    # Topraklama raporlarında MUTLAKA olması gereken kritik kelimeler
    critical_terms = [
         # Topraklama temel terimleri
        ["topraklama", "grounding", "earthing", "earth", "zs", "r değeri", "toprak direnci"],
        
        # Topraklama ölçüm birimleri ve değerleri
        ["ohm", "ω", "milliohm", "mω", "direnç", "resistance", "impedans", "impedance"],
        
        # EN 60204-1 standardı ve şebeke tipleri
        ["en 60204", "60204", "tn", "tt", "it", "şebeke", "network", "system"],
        
        # Koruma cihazları ve ölçüm
...
def check_excluded_keywords_first_pages(filepath):
    ...
    excluded_keywords = [
        # Aydınlatma raporu (eski strong_keywords)
    ...
    try:
        if 'file' not in request.files:
            return jsonify({
                ...
                'message': 'Lütfen analiz edilmek üzere bir topraklama raporu sağlayın'
            }), 400
        ...
        try:
                    ...
                    if check_excluded_keywords_first_pages(filepath):
                        ...
                        return jsonify({
                            ...
                            'details': {
                                ...
                                'required_type': 'TOPRAKLAMA_OLCUM_RAPORU'
                            }
                        }), 400
                    else:
                        ...
                        try:
                            ...
                            if not validate_document_server(text):
                                ...
                                return jsonify({
                                    ...
                                    'details': {
                                        'filename': filename,
                                        'document_type': 'NOT_TOPRAKLAMA_REPORT',
                                        'required_type': 'TOPRAKLAMA_OLCUM_RAPORU'
                                    }
                                }), 400
            ...
            # Buraya kadar geldiyse topraklama raporu, şimdi analizi yap
            ...
            response_data = {
                ...
                'filename': filename,
                'language_info': {
                    'detected_language': 'turkish',#map_language_code(analysis_result['dosya_bilgisi']['detected_language']),
                    'text_length': analysis_result.get('text_length', 0)
                },
                'overall_score': {
...
def health_check():
    """Health check endpoint - Topraklama için"""
    return jsonify({
        'status': 'healthy',
        ...
        'ocr_support': True,  # Topraklama'da OCR var
        'report_type': 'TOPRAKLAMA_OLCUM_RAPORU'
    })

@app.route('/api/test-topraklama', methods=['GET'])
def test_topraklama_analysis():
    """Test endpoint for debugging - Topraklama için"""
    try:
        analyzer = EN60204TopraklamaRaporuAnaliz()
        test_info = {
...
@app.route('/api/topraklama-categories', methods=['GET'])
def get_topraklama_categories():
    """Topraklama analiz kategorilerini döndür - Topraklama formatında"""
    try:
        analyzer = EN60204TopraklamaRaporuAnaliz()
        return jsonify({
            'success': True,
            'data': {
                ...
                'standard_reference': 'EN 60204-1 Makinalarda Elektrik Donanımı'
            }
        })
    except Exception as e:
        return jsonify({
...
def index():
    """Ana sayfa - API bilgileri - Topraklama için"""
    return jsonify({
        'service': 'Grounding Report Analyzer API',
        'version': '1.0.0',
        ...
        'endpoints': {
            ...
            'GET /api/test-topraklama': 'Test analizi',
            'GET /api/topraklama-categories': 'Analiz kategorileri',
        ...
        'example_curl': 'curl -X POST -F "file=@topraklama_raporu.pdf" http://localhost:5014/api/topraklama-report'
    })

if __name__ == '__main__':
    logger.info("Grounding Report Analyzer API başlatılıyor...")
    logger.info(f"Upload klasörü: {UPLOAD_FOLDER}")
    logger.info("API Endpoints:")
    logger.info("  POST /api/topraklama-report - Topraklama raporu analizi")
    logger.info("  GET /api/health - Servis sağlık kontrolü")
    logger.info("  GET /api/test-topraklama - Test analizi")
    logger.info("  GET /api/topraklama-categories - Analiz kategorileri")
    logger.info("  GET / - API bilgileri")

    assigned_port = int(os.environ.get('ASSIGNED_PORT', 5014))
