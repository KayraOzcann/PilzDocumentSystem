...
from aydinlatma3 import LightingReportAnalyzer
import logging
...
def validate_document_server(text):
    ...
    # Aydınlatma raporlarında MUTLAKA olması gereken kritik kelimeler
    critical_terms = [
...
def analyze_aydinlatma_report():
    ...
    try:
        ...
        try:
            ...
            analysis_result = analyzer.analyze_lighting_report(filepath)
            ...
            response_data = {
                ...
                'summary': {
                    ...
                    'main_issues': [] if status == "PASS" else get_main_issues_aydinlatma(analysis_result)
                }
            }
...
def get_conclusion_message_aydinlatma(status, percentage):
    ...
    if status == "PASS":
        ...
        return f"Aydınlatma raporu kabul edilebilir ancak bazı eksiklikler var (%{percentage:.0f})"
    else:
        return f"Aydınlatma raporu standartlara uygun değil, kapsamlı revizyon gerekli (%{percentage:.0f})"

def get_main_issues_aydinlatma(analysis_result):
    ...
    if not issues:
        if analysis_result['puanlama']['total_score'] < 50:
            issues = [
                ...
                "TS EN 12464-1 standard kontrolü eksik"
            ]
...
def health_check():
    ...
    return jsonify({
        ...
        'ocr_support': True,  # Aydınlatma'da OCR var
        'report_type': 'AYDINLATMA_OLCUM_RAPORU'
    })

@app.route('/api/test-aydinlatma', methods=['GET'])
def test_aydinlatma_analysis():
    """Test endpoint for debugging - Aydınlatma için"""
    try:
        analyzer = LightingReportAnalyzer()
        test_info = {
...
@app.route('/api/aydinlatma-categories', methods=['GET'])
def get_aydinlatma_categories():
    """Aydınlatma analiz kategorilerini döndür - Topraklama formatında"""
    try:
        analyzer = LightingReportAnalyzer()
        return jsonify({
            'success': True,
            'data': {
                ...
                'standard_reference': 'TS EN 12464-1 ve İSG mevzuatı'
            }
        })
    except Exception as e:
        return jsonify({
...
def index():
    ...
    return jsonify({
        ...
        'endpoints': {
            ...
            'GET /api/test-aydinlatma': 'Test analizi',
            'GET /api/aydinlatma-categories': 'Analiz kategorileri',
        ...
        'example_curl': 'curl -X POST -F "file=@aydinlatma_raporu.pdf" http://localhost:5018/api/aydinlatma-report'
    })

if __name__ == '__main__':
    logger.info("Lighting Report Analyzer API başlatılıyor...")
    ...
    logger.info("  GET /api/test-aydinlatma - Test analizi")
    logger.info("  GET /api/aydinlatma-categories - Analiz kategorileri")
    logger.info("  GET / - API bilgileri")

    assigned_port = int(os.environ.get('ASSIGNED_PORT', 5018))
